...
class Networks():
    ...
    def list_networks(self, platform, group_id=None):
        service_url = "/v2/%s/networks/" % platform
        params = {'project': group_id}
        return self.req_get(url=service_url, params=params)

    def create_network(self, name, cidr, gateway, group_id,
                       platform, with_router=False):
        service_url = "/v2/%s/networks/" % platform
        data = {'name': name, 'cidr': cidr,
                'gateway': gateway, 'project': group_id,
                'with_router': with_router}
        return self.req_post(url=service_url,
                             data=data)

    def delete_network(self, network_id, platform):
        service_url = "/v2/%(platform)s/networks/%(network_id)s/"
        service_url = service_url % ({'network_id': network_id,
                                      'platform': platform})
        return self.req_delete(url=service_url)

    def get_detail(self, network_id, platform):
        service_url = "/v2/%(platform)s/networks/%(network_id)s/"
        service_url = service_url % ({'network_id': network_id,
                                      'platform': platform})
        return self.req_get(url=service_url)
