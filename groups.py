...
class Groups():
    ...
    def create_group(self,
                     ...
                     desc="",
                     sp_admin=None,
                     sp_admin_passwd=None):
        service_url = "/v2/%s/projects/" % platform_name
        data = {'name': name,
                'desc': desc,
                'sp_admin': sp_admin,
                'sp_admin_passwd': sp_admin_passwd}
        return self.req_post(url=service_url,
                             data=data)

    def list_groups(self):
        service_url = "/v2/projects/"
        return self.req_get(url=service_url)

    def get_detail(self, group_id):
        service_url = "/v2/projects/%(group_id)s/"
        service_url = service_url % ({'group_id': group_id})
        return self.req_get(url=service_url)

    def get_all_groups(self, project_name):
        service_url = "/v2/projects/%(group_name)s/"
        service_url = service_url % ({'group_name': project_name})
        return self.req_get(url=service_url)

    def get_solutions(self, group_id):
        service_url = "/v2/projects/%(group_id)s/solutions/"
        service_url = service_url % ({'group_id': group_id})
        return self.req_get(url=service_url)
    ...
    def add_solution(self, group_id, solution_id):
        service_url = "/v2/projects/%(group_id)s/solutions/%(solution_id)s/"
        service_url = service_url % ({'group_id': group_id,
    ...
    def update_quota(self, platform, group_id, quotas):
        service_url = "/v2/%(platform)s/projects/%(group_id)s/quota/"
        service_url = service_url % ({'group_id': group_id,
                                      'platform': platform})
    ...
    def get_assets_reports(self, group_id=None,
        ...
        service_url = "/v2/projects/reports/assets/"
    ...
    def get_statistics_reports(self, group_id=None,
                               begin_time=None, end_time=None):
        service_url = "/v2/projects/reports/statistics/"
        params = {'group_id': group_id}
        if begin_time:
            params.update({'begin_time': begin_time})
        if end_time:
