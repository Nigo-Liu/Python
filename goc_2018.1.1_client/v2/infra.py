...
class Infra():
    ...
    def list_nodes(self, hostname=None, role=None):
        ...
        if hostname and role:
            service_url = "/v2/infrastructure/hosts/?%s&%s" % (name_query,
                                                               role_query)
        elif hostname or role:
            service_url = "/v2/infrastructure/hosts/?%s" %\
                (name_query or role_query)
        else:
            service_url = "/v2/infrastructure/hosts/"
        return self.req_get(url=service_url)

    def get_node_detail(self, host_id):
        service_url = "/v2/infrastructure/hosts/%s/" % host_id
        return self.req_get(url=service_url)

    def get_pods_on_node(self, host_id):
        service_url = "/v2/infrastructure/hosts/%s/pods/" % host_id
        return self.req_get(url=service_url)

    def list_networks(self):
        service_url = "/v2/infrastructure/network/"
        return self.req_get(url=service_url)
